...
node {
   ...
   sh "/apache-maven-3.2.1/bin/mvn versions:set -DnewVersion=${env.BUILD_NUMBER}"
   sh "/apache-maven-3.2.1/bin/mvn package"

   stage 'test'
   parallel 'test': {
     sh "/apache-maven-3.2.1/bin/mvn test; sleep 2;"
   }, 'verify': {
     sh "/apache-maven-3.2.1/bin/mvn verify; sleep 3"
   }
